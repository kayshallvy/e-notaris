...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">
            <i class="bi bi-pencil-square"></i> Edit Klien Akta
        </h3>
    </div>
    <div class="card-body">
        <form action="{{ route('akta-klien.update', ['aktaKlien' => $aktaKlien->id_akta_klien]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('id_akta') is-invalid @enderror" 
                        ...
                        @foreach($aktas as $akta)
                            <option value="{{ $akta->id_akta }}" {{ old('id_akta', $aktaKlien->id_akta) == $akta->id_akta ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('id_klien') is-invalid @enderror" 
                        ...
                        @foreach($kliens as $klien)
                            <option value="{{ $klien->id_klien }}" {{ old('id_klien', $aktaKlien->id_klien) == $klien->id_klien ? 'selected' : '' }}>
                                {{ $klien->nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('peran') is-invalid @enderror" 
                       id="peran" name="peran" value="{{ old('peran', $aktaKlien->peran) }}" required>
                @error('peran')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
