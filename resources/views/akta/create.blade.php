                    ...
                    <form action="{{ route('akta.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="nomor_akta" id="nomor_akta" value="{{ old('nomor_akta') }}" required 
                            ...
                            <div>
                                ...
                                <input type="text" name="jenis_akta" id="jenis_akta" value="{{ old('jenis_akta') }}" required
                            ...
                            <div>
                                ...
                                <select name="id_notaris" id="id_notaris" required
                                    ...
                                    @foreach($notarises as $notaris)
                                        <option value="{{ $notaris->id }}" {{ old('id_notaris') == $notaris->id ? 'selected' : '' }}>
                                            {{ $notaris->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="tanggal_dibuat" id="tanggal_dibuat" value="{{ old('tanggal_dibuat') }}" required
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="keterangan" id="keterangan" rows="4"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('keterangan') border-red-500 @enderror">{{ old('keterangan') }}</textarea>
                            @error('keterangan')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
